...
if resume:
    df0 = pd.read_csv(datadir+'ere_database_pfs.csv')
    df0['time'] = pd.to_datetime(df0['time'], format="%Y%m%d%H")
    # save parameters for events before run period
    df0 = df0[df0.time < datetime(startyear,startmonth,1)]
    df0['time'] = df0['time'].dt.strftime('%Y%m%d%H')
...
for event in events:
    ...
    df_pfs.to_csv(datadir+'ere_database_pfs.csv',index=False)
